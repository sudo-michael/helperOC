function [traj, traj_tau, u_hist, v_hist] = randomTraj(g, data, tau, dynSys, extraArgs)
...
traj(:,1) = dynSys.x;
u_hist = nan(1, tauLength);
u_hist(:, 1) = 0;
v_hist = nan(1, tauLength);
v_hist(:, 1) = 0;
...
%   tEarliest = find_earliest_BRS_ind(g, data, dynSys.x, upper, lower);
  ...
  for j = 1:subSamples
    ...
    value = eval_u(g, BRS_at_t, dynSys.x);
